...
type ChunkPreview = { id: string; parent_id: string; parent_sequence: number; source_page: number; source_sheet?: string; source_cell_range?: string; sequence: number; heading_path?: string[]; content: string; parent_content: string };
type ChunkPreviewResponse = { chunker_version: string; max_runes: number; overlap_runes: number; overlap_ratio: number; parent_count: number; child_count: number; parent_runes: number; indexed_runes: number; embedding_amplification: number; pages: number[]; chunks: ChunkPreview[] };
type AnswerResponse = {
  answerable: boolean;
  answer: string;
...
type AgentAnswerResponse = { app_id: string; environment_id: string; thread_id?: string; result: { status: string; answer: string; answer_source?: string; citations?: Array<{ chunk_id: string; document_id: string; document: string; excerpt: string }>; needs_confirmation?: boolean; confirmation_id?: string; steps: AgentStep[]; tool_calls?: string[] } };
type ChatMessage = { id: string; role: "user" | "assistant"; text: string; response?: AnswerResponse; pending?: boolean };
type AgentChatMessage = { id: string; role: "user" | "assistant"; text: string; query?: string; response?: AgentAnswerResponse; pending?: boolean };
...
const AGENT_API_BASE = process.env.NEXT_PUBLIC_AGENT_API_BASE ?? "http://127.0.0.1:8090";
const DEMOS = [
...
export default function CustomerPortal() {
  ...
  const [chunkProfile, setChunkProfile] = useState({ max_runes: 700, overlap_runes: 80 });
  ...
  const [agentPendingQuery, setAgentPendingQuery] = useState("");
  ...
  async function agentApi(path: string, init: RequestInit = {}, token = session?.access_token) {
    const headers = new Headers(init.headers);
    headers.set("Content-Type", "application/json");
    if (token) headers.set("Authorization", `Bearer ${token}`);
    const response = await fetch(`${AGENT_API_BASE}${path}`, { ...init, headers });
    if (!response.ok) throw new Error(await parseError(response));
    return response;
  }
  ...
  function logout() {
    ...
    window.localStorage.removeItem("raglab-portal-session");
    setSession(null); setMessages([]); setDatasets([]); setSelectedDataset(""); setApplications([]); setSelectedApplication(""); setEnvironments([]); setSelectedEnvironment(""); setAppAnswer(null); setAgentAnswer(null); setAgentMessages([]); setAgentPendingQuery("");
  }
  ...
  async function askAgent() {
    ...
    const userId = uid("agent-user");
    const assistantId = uid("agent-assistant");
    setAgentQuery("");
    setAgentBusy(true); setNotice("");
    setAgentMessages((current) => [...current, { id: userId, role: "user", text }, { id: assistantId, role: "assistant", text: "正在规划任务与检查可用工具…", query: text, pending: true }]);
    try {
      const response = await agentApi(`/api/v1/apps/${encodeURIComponent(selectedApplication)}/agent/answer`, {
        method: "POST", body: JSON.stringify({ environment_id: selectedEnvironment, query: text }),
      });
      const result = (await response.json()) as AgentAnswerResponse;
      setAgentAnswer(result);
      if (result.result.needs_confirmation) setAgentPendingQuery(text);
      setAgentMessages((current) => current.map((message) => message.id === assistantId ? { ...message, text: result.result.answer, response: result, pending: false } : message));
    } catch (error) {
      const message = error instanceof Error ? error.message : "Agent 执行失败";
      setNotice(message);
      setAgentMessages((current) => current.map((item) => item.id === assistantId ? { ...item, text: `这次请求没有完成：${message}`, pending: false } : item));
    } finally {
      setAgentBusy(false);
    }
  }

  async function confirmAgent(targetResponse?: AgentAnswerResponse, targetQuery?: string) {
    const pendingResponse = targetResponse ?? agentAnswer;
    const pendingQuery = targetQuery ?? agentPendingQuery;
    if (!pendingResponse?.thread_id || !pendingQuery || !selectedApplication || !selectedEnvironment || agentConfirming) return;
    const userId = uid("agent-confirm");
    const assistantId = uid("agent-confirmation");
    setAgentMessages((current) => [...current, { id: userId, role: "user", text: "确认提交这份工单草稿" }, { id: assistantId, role: "assistant", text: "正在提交确认…", query: pendingQuery, pending: true }]);
    setAgentConfirming(true); setNotice("");
    try {
      const response = await agentApi(`/api/v1/apps/${encodeURIComponent(selectedApplication)}/agent/answer`, {
        method: "POST",
        body: JSON.stringify({ environment_id: selectedEnvironment, query: pendingQuery, thread_id: pendingResponse.thread_id, confirmation: true }),
      });
      const result = (await response.json()) as AgentAnswerResponse;
      setAgentAnswer(result);
      setAgentPendingQuery(result.result.needs_confirmation ? pendingQuery : "");
      setAgentMessages((current) => current.map((message) => {
        if (message.id === assistantId) return { ...message, text: result.result.answer, response: result, pending: false };
        if (message.response?.thread_id === pendingResponse.thread_id) return { ...message, response: { ...message.response, result: { ...message.response.result, needs_confirmation: false } } };
        return message;
      }));
    } catch (error) {
      const message = error instanceof Error ? error.message : "工单确认失败";
      setNotice(message);
      setAgentMessages((current) => current.map((item) => item.id === assistantId ? { ...item, text: `确认没有完成：${message}`, pending: false } : item));
    } finally {
  ...
  useEffect(() => {
    ...
    setAgentMessages([]);
    setAgentPendingQuery("");
  }, [selectedApplication, selectedEnvironment]);
  ...
  async function previewDocument() {
    ...
    try {
      const response = await api(`/api/v1/datasets/${selectedDataset}/documents/preview`, {
        method: "POST",
        body: JSON.stringify({ title: document.title, content: document.content, ...chunkProfile }),
      });
    ...
    <main className="portal-shell">
      <aside className={`portal-sidebar ${sidebarOpen ? "open" : ""}`}>
        ...
        <nav className="portal-nav">
          {isAdmin && <button className={view === "agent" ? "active" : ""} onClick={() => { setView("agent"); setSidebarOpen(false); }}><span>✦</span>企业 Agent 工作台<em>DEEPSEEK</em></button>}
      ...
      <section className="portal-main">
        <header className="portal-header"><button className="mobile-menu" onClick={() => setSidebarOpen((open) => !open)}>☰</button><div><p className="eyebrow">AGENT OPERATIONS / RAG</p><h1>{view === "agent" ? "企业 Agent 工作台" : view === "apps" ? "Agent 应用" : view === "runtime" ? "运行控制面" : view === "knowledge" ? "知识库" : view === "ingest" ? "导入资料" : "权限与审计"}</h1></div><div className="header-actions"><span className="api-status"><i />API 在线</span><button className="avatar" title={session.identity.subject}>{session.identity.subject.slice(-2).toUpperCase()}</button></div></header>
...
function AgentView(props: { applications: AgentApplication[]; environments: AppEnvironment[]; selectedApplication: string; setSelectedApplication: (value: string) => void; selectedEnvironment: string; setSelectedEnvironment: (value: string) => void; query: string; setQuery: (value: string) => void; messages: AgentChatMessage[]; answer: AgentAnswerResponse | null; busy: boolean; ask: () => void; confirming: boolean; confirm: (response?: AgentAnswerResponse, query?: string) => void; clear: () => void }) {
  const latestMessage = [...props.messages].reverse().find((message) => message.response);
  const latest = latestMessage?.response ?? props.answer;
  const conversationRef = useRef<HTMLDivElement>(null);
  const stickToBottomRef = useRef(true);
  const messageCountRef = useRef(props.messages.length);
  const lastMessage = props.messages[props.messages.length - 1];
  const lastMessageKey = lastMessage ? `${lastMessage.id}:${lastMessage.text.length}:${lastMessage.pending ? "pending" : "complete"}` : "empty";

  useEffect(() => {
    const element = conversationRef.current;
    if (!element) return;
    const messageCountChanged = props.messages.length !== messageCountRef.current;
    const frame = window.requestAnimationFrame(() => {
      // New turns land at the latest answer. During streaming, keep the
      // viewport pinned only when the user was already following the answer.
  ...
  }, [lastMessageKey, props.messages.length]);
  ...
  if (!props.applications.length) {
    return <div className="content-page application-page"><div className="page-intro"><div><span className="section-kicker">CONTROLLED AGENT LOOP / LANGGRAPH + DEEPSEEK</span><h2>企业 Agent 工作台</h2><p>先创建 Application 并绑定知识库，Agent 才能在服务端完成身份校验、检索和工具调用。</p></div><span className="secure-pill">✓ TOOL POLICY ENFORCED</span></div><div className="empty-evidence app-empty"><div>✦</div><strong>还没有可用的 Agent 应用</strong><p>先通过 Agent 应用控制面创建 Application，再回来体验完整对话。</p></div></div>;
  }
  return <div className="chat-layout agent-chat-layout"><div className="chat-column"><div className="workspace-strip"><div><span className="section-kicker">CONTROLLED AGENT LOOP / LANGGRAPH + DEEPSEEK</span><strong>企业 Agent 工作台</strong><small>LangGraph 管理状态与人工确认；LangChain 适配 DeepSeek、工具和 Go 授权 RAG。</small></div><div className="agent-scope-selectors"><label>应用<select value={props.selectedApplication} onChange={(event) => props.setSelectedApplication(event.target.value)}>{props.applications.map((application) => <option key={application.app_id} value={application.app_id}>{application.name} · {application.tenant_id}</option>)}</select></label><label>环境<select value={props.selectedEnvironment} onChange={(event) => props.setSelectedEnvironment(event.target.value)}>{props.environments.map((environment) => <option key={environment.environment_id} value={environment.environment_id}>{environment.name} · {environment.config_version}</option>)}</select></label></div></div><div className="conversation" ref={conversationRef} onScroll={handleConversationScroll}><div className="welcome-card"><div className="spark">✦</div><h2>你好，我是企业 Agent</h2><p>我可以回答知识问题、查询服务状态、解释当前权限，也可以先生成工单草稿。涉及写操作时会暂停等待你的确认，不会绕过服务端权限。</p><div className="suggestions"><button type="button" onClick={() => props.setQuery("如何申请企业单点登录？")}>如何申请企业单点登录？</button><button type="button" onClick={() => props.setQuery("当前服务状态怎么样？")}>当前服务状态怎么样？</button><button type="button" onClick={() => props.setQuery("我的权限是什么？")}>我的权限是什么？</button><button type="button" onClick={() => props.setQuery("帮我创建一个登录故障工单")}>创建登录故障工单</button></div></div>{props.messages.map((message) => <div className={`message-row ${message.role}`} key={message.id}><div className="message-avatar">{message.role === "user" ? "我" : "A"}</div><div className="message-body"><div className="message-meta">{message.role === "user" ? "你" : "企业 Agent"}<span>{message.role === "assistant" && message.pending ? "执行中" : message.response ? message.response.result.status === "needs_confirmation" ? "等待确认" : "已完成" : ""}</span></div><div className={`message-bubble ${message.pending ? "pending" : ""}`}>{message.text}{message.pending && <i className="typing" />}</div>{message.response && <AgentAnswerMeta response={message.response} query={message.query ?? ""} confirming={props.confirming} confirm={props.confirm} />}</div></div>)}</div><form className="composer" onSubmit={(event) => { event.preventDefault(); props.ask(); }}><textarea value={props.query} onChange={(event) => props.setQuery(event.target.value)} placeholder="向 Agent 描述你的业务请求…" rows={2} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); props.ask(); } }} /><div className="composer-bottom"><span>↗ Enter 发送 · Shift + Enter 换行 · 当前请求受应用/环境策略约束</span><div><button type="button" className="ghost-button" onClick={props.clear} disabled={props.busy || props.confirming}>新会话</button><button className="send-button" disabled={props.busy || props.confirming || !props.query.trim() || !props.selectedEnvironment}>{props.busy ? "Agent 执行中…" : "发送"} <span>↑</span></button></div></div></form></div><aside className="evidence-panel"><AgentTracePanel response={latest} query={latestMessage?.query ?? ""} confirming={props.confirming} confirm={props.confirm} /><div className="panel-footnote"><span>●</span> 对话只展示当前应用与环境可见的数据；所有工具调用均由服务端策略校验。</div></aside></div>;
}

function AgentAnswerMeta({ response, query, confirming, confirm }: { response: AgentAnswerResponse; query: string; confirming: boolean; confirm: (response?: AgentAnswerResponse, query?: string) => void }) {
  const result = response.result;
  return <div className="answer-meta agent-answer-meta"><div className="answer-stats"><span>{result.needs_confirmation ? "等待用户确认" : result.answer_source === "rag" ? "RAG 引用回答" : result.answer_source === "tool" ? "受控工具回答" : result.answer_source === "confirmation" ? "写操作确认" : "通用服务台回复"}</span><span>{result.steps.length} steps</span><span>{result.tool_calls?.join(" · ") || "无工具调用"}</span></div>{result.needs_confirmation && <div className="agent-confirm-row"><span>工单草稿尚未写入业务系统</span><button type="button" className="primary-button compact" onClick={() => confirm(response, query)} disabled={confirming || !response.thread_id}>{confirming ? "确认中…" : "确认提交工单"}</button></div>}{result.citations?.length ? <div className="citation-list"><span>引用来源</span>{result.citations.slice(0, 3).map((citation) => <button type="button" key={citation.chunk_id} title={citation.excerpt}>⌕ {citation.document}</button>)}</div> : null}</div>;
}

function AgentTracePanel({ response, query, confirming, confirm }: { response: AgentAnswerResponse | null; query: string; confirming: boolean; confirm: (response?: AgentAnswerResponse, query?: string) => void }) {
  if (!response) return <><div className="panel-heading"><div><span className="section-kicker">AGENT TRACE</span><h3>执行轨迹</h3></div><span className="count-badge">—</span></div><div className="empty-evidence"><div>⌁</div><strong>等待一次对话</strong><p>发送业务请求后，这里会展示 Agent 的规划、工具调用、权限边界和引用。</p></div></>;
  const result = response.result;
  return <><div className="panel-heading"><div><span className="section-kicker">AGENT TRACE</span><h3>本轮执行轨迹</h3></div><span className="count-badge">{result.steps.length}</span></div><div className="agent-trace-summary"><span className={`task-status ${result.status}`}>{result.status === "needs_confirmation" ? "待确认" : result.status === "completed" ? "已完成" : "需澄清"}</span><small>{result.answer_source === "rag" ? "通过 Go Gateway 检索" : "工具 / 人设路径"}</small></div><div className="binding-proof agent-trace-list">{result.steps.map((step) => <div className="binding-proof-row" key={step.step}><strong>Step {step.step} · {step.action.type}{step.action.tool ? ` / ${step.action.tool}` : ""}</strong><span>{step.action.reason || step.action.message || "planner decision"}</span>{step.observation && <small>{step.observation.status} · {step.observation.summary || "observation recorded"}</small>}</div>)}</div>{result.tool_calls?.length ? <div className="agent-tool-strip"><span>TOOLS</span>{result.tool_calls.map((tool) => <code key={tool}>{tool}</code>)}</div> : null}{result.needs_confirmation && <button type="button" className="primary-button compact agent-trace-confirm" onClick={() => confirm(response, query)} disabled={confirming || !response.thread_id}>{confirming ? "确认中…" : "确认工单草稿"}</button>}{result.citations?.length ? <div className="citation-list agent-trace-citations"><span>引用</span>{result.citations.slice(0, 5).map((citation) => <button type="button" key={citation.chunk_id} title={citation.excerpt}>⌕ {citation.document}</button>)}</div> : null}</>;
}
...
function IngestView(props: { datasets: Dataset[]; selected: string; setSelected: (value: string) => void; isPlatformAdmin: boolean; document: { document_id: string; title: string; content: string; version: string; source_revision: string }; setDocument: (document: { document_id: string; title: string; content: string; version: string; source_revision: string }) => void; chunkProfile: { max_runes: number; overlap_runes: number }; setChunkProfile: (profile: { max_runes: number; overlap_runes: number }) => void; loadFile: (file: File) => void; submit: (event: FormEvent) => void; importing: boolean; preview: () => void; previewBusy: boolean; chunkPreview: ChunkPreviewResponse | null }) {
  const writableDatasets = props.isPlatformAdmin ? props.datasets : props.datasets.filter((dataset) => dataset.visibility === "tenant");
  return <div className="content-page"><div className="page-intro"><div><span className="section-kicker">KNOWLEDGE LIFECYCLE</span><h2>导入一份新资料</h2><p>先用结构化预览检查标题、页码、父子 Chunk 和重叠范围，再执行 Embedding、Milvus Upsert 与读回校验。</p></div><span className="pipeline-pill"><i /> PREVIEW → VALIDATE → EMBED → INDEX</span></div><form className="ingest-panel" onSubmit={props.submit}><div className="ingest-target"><div><span className="section-kicker">TARGET DATASET</span><strong>{writableDatasets.find((dataset) => dataset.id === props.selected)?.name ?? "选择知识库"}</strong></div><select value={props.selected} onChange={(event) => props.setSelected(event.target.value)} required>{writableDatasets.map((dataset) => <option key={dataset.id} value={dataset.id}>{dataset.name}</option>)}</select></div><div className="form-grid"><label className="wide">导入 Markdown / 文本文件<input type="file" accept=".md,.markdown,.txt,text/markdown,text/plain" onChange={(event) => { const file = event.target.files?.[0]; if (file) props.loadFile(file); event.currentTarget.value = ""; }} /><small>浏览器本地读取；演示版单份资料最大 64 KB。PDF 页码接入将在解析器适配层统一转换为 page marker。</small></label><label>文档 ID<input value={props.document.document_id} onChange={(event) => props.setDocument({ ...props.document, document_id: event.target.value })} placeholder="support-sso-v1" required /></label><label>版本<input value={props.document.version} onChange={(event) => props.setDocument({ ...props.document, version: event.target.value })} placeholder="v1" /></label><label>源修订号<input type="number" min="1" value={props.document.source_revision} onChange={(event) => props.setDocument({ ...props.document, source_revision: event.target.value })} required /></label><label>Child Chunk 字符<input type="number" min="100" max="2000" value={props.chunkProfile.max_runes} onChange={(event) => props.setChunkProfile({ ...props.chunkProfile, max_runes: Number(event.target.value) })} /></label><label>Overlap 字符<input type="number" min="0" max={Math.max(0, Math.floor(props.chunkProfile.max_runes / 2) - 1)} value={props.chunkProfile.overlap_runes} onChange={(event) => props.setChunkProfile({ ...props.chunkProfile, overlap_runes: Number(event.target.value) })} /><small>先从 10%–15% 候选开始，再用 Golden Cases 决定。</small></label><label className="wide">标题<input value={props.document.title} onChange={(event) => props.setDocument({ ...props.document, title: event.target.value })} placeholder="企业单点登录接入指南" required /></label><label className="wide">正文<textarea value={props.document.content} onChange={(event) => props.setDocument({ ...props.document, content: event.target.value })} placeholder="粘贴 Markdown 或纯文本内容。建议包含清晰的小标题和步骤…" rows={12} required /></label></div><div className="ingest-actions"><span>当前 {props.chunkProfile.max_runes} 字符 Chunk / {props.chunkProfile.overlap_runes} 字符 overlap · 预览不写 Milvus</span><div><button type="button" className="ghost-button" onClick={props.preview} disabled={props.previewBusy || !props.document.title.trim() || !props.document.content.trim()}>{props.previewBusy ? "预览中…" : "预览分块"}</button><button className="primary-button" disabled={props.importing || writableDatasets.length === 0}>{props.importing ? "正在入库…" : "导入并验证"}<span>↑</span></button></div></div></form>{props.chunkPreview && <ChunkPreviewPanel preview={props.chunkPreview} />}</div>;
}

function ChunkPreviewPanel({ preview }: { preview: ChunkPreviewResponse }) {
  return <section className="chunk-preview-panel"><div className="panel-heading"><div><span className="section-kicker">STRUCTURED CHUNK INSPECTOR</span><h3>分块与溯源预览</h3></div><span className="secure-pill">{preview.chunker_version}</span></div><div className="chunk-preview-metrics"><span><b>{preview.parent_count}</b> parents</span><span><b>{preview.child_count}</b> children</span><span><b>{preview.pages.length || 1}</b> pages</span><span><b>{preview.max_runes}/{preview.overlap_runes}</b> max / overlap</span><span><b>{(preview.overlap_ratio * 100).toFixed(1)}%</b> overlap ratio</span><span><b>{preview.embedding_amplification.toFixed(2)}×</b> embedding text</span></div><p className="chunk-preview-note">父 Chunk 保留完整逻辑段落，子 Chunk 用于召回；页码、工作表、单元格范围与标题路径用于后续引用定位。Embedding text 倍率用于观察 overlap 和重复标题带来的成本放大，不代表最终 Token 账单。</p><div className="chunk-preview-list">{preview.chunks.slice(0, 12).map((chunk) => <article className="chunk-preview-card" key={chunk.id}><div className="chunk-preview-card-head"><strong>{chunk.id}</strong><span>{chunk.source_sheet ? `${chunk.source_sheet}!${chunk.source_cell_range || "—"}` : `page ${chunk.source_page || "—"}`} · parent {chunk.parent_sequence}</span></div><small>{chunk.heading_path?.join(" / ") || "无标题路径"} · {chunk.content.length} chars</small><p>{chunk.content.slice(0, 260)}{chunk.content.length > 260 ? "…" : ""}</p><code>{chunk.parent_id}</code></article>)}</div>{preview.chunks.length > 12 && <small className="chunk-preview-more">仅展示前 12 个 child Chunk，完整结果由 API 返回。</small>}</section>;
}
