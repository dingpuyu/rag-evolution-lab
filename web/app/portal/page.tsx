...
export default function CustomerPortal() {
  ...
  async function confirmAgent() {
    if (!agentAnswer?.thread_id || !selectedApplication || !selectedEnvironment || agentConfirming) return;
    setAgentConfirming(true); setNotice("");
    try {
      const response = await agentApi(`/api/v1/apps/${encodeURIComponent(selectedApplication)}/agent/answer`, {
        method: "POST",
        body: JSON.stringify({ environment_id: selectedEnvironment, query: agentQuery.trim(), thread_id: agentAnswer.thread_id, confirmation: true }),
      });
      setAgentAnswer((await response.json()) as AgentAnswerResponse);
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "工单确认失败");
    } finally { setAgentConfirming(false); }
  }
  ...
  return (
    <main className="portal-shell">
      ...
      <section className="portal-main">
        ...
        {view === "agent" && <AgentView applications={applications} environments={environments} selectedApplication={selectedApplication} setSelectedApplication={setSelectedApplication} selectedEnvironment={selectedEnvironment} setSelectedEnvironment={setSelectedEnvironment} query={agentQuery} setQuery={setAgentQuery} answer={agentAnswer} busy={agentBusy} ask={() => void askAgent()} confirming={agentConfirming} confirm={() => void confirmAgent()} />}
