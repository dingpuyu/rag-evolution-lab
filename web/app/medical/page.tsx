...
const AGENT_EVALUATION_URL = process.env.NEXT_PUBLIC_AGENT_EVALUATION_URL ?? "http://127.0.0.1:18200";
...
function BadCaseWorkbench({ cases, busy, edit, save, verify, promote }: { cases: BadCase[]; busy: string; edit: (id: string, patch: Partial<BadCase>) => void; save: (item: BadCase) => void; verify: (item: BadCase) => void; promote: (item: BadCase) => void }) {
  ...
  return <div className="medical-content medical-badcase-workbench">
    ...
    {cases.length === 0 ? <div className="medical-empty">当前应用还没有人工确认的 Bad Case。评测失败后可在上方逐题收录。</div> : <section className="medical-badcase-list">{cases.map((item) => <article key={item.bad_case_id}>
      ...
      <footer><a className="medical-external-link" href={AGENT_EVALUATION_URL} target="_blank" rel="noreferrer">外部证据诊断 ↗</a><button className="secondary" disabled={busy === item.bad_case_id} onClick={() => save(item)}>保存人工诊断</button>{item.layer === "rag" && item.status !== "regression" && <button disabled={busy === item.bad_case_id} onClick={() => verify(item)}>写入单题复测</button>}{item.status === "verified" && <button disabled={busy === item.bad_case_id} onClick={() => promote(item)}>晋升回归集</button>}{item.status === "regression" && <b>回归资产已保留</b>}</footer>
    </article>)}</section>}
  </div>;
}
