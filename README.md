...
在新电脑部署完整医疗设备 Agent 与配套评测平台，请直接使用[可移植双平台部署](docs/portable-deployment.md)。部署引导会生成真实随机账号和基础设施凭据，模型 Key 只从进程环境注入。
...
| Development 全绿但未见低对比扫描 | 一次性 Holdout 实跑 `2/3`，发现 Paddle `block_order=null` 使 Adapter 在排序时抛出 `int/None` 异常；同候选重复提交被 `409` 阻止 | 对整页可选顺序字段做防御性归一化；任一顺序缺失时使用 bbox 视觉顺序；新增不同型号 Development Bad Case，Worker 版本进入指纹 | 新 Development `4/5 → 5/5`；原 Holdout 失败保留，不用已见题复跑；Parser 新增 OCR 依赖 `/readyz`，Worker 下线实测返回 `503` |
...
P2 Knowledge Gateway 已接入查询链路：

- `POST /api/v1/apps/{app_id}/query`：按 Application + Environment 解析多知识库绑定，执行服务端 ACL、策略化候选召回、去重和全局 Top-K；
- `POST /api/v1/apps/{app_id}/answer`：在同一应用边界上生成带 Citation Allowlist 的 grounded answer；
- 上层 Agent 不再提交 `tenant_id`、`product` 或 Milvus Filter，跨租户、撤权和无绑定环境均 fail-closed；
- 原 `/api/v1/datasets/{dataset_id}/...` 接口继续兼容，作为单知识库运维与迁移入口。
